...
function nodeIncludesCodeElem<T extends React.ReactNode>(children: T): [(T extends Iterable<React.ReactNode> ? Array<React.ReactNode> : T), boolean | undefined] {
  ...
  if(Symbol.iterator in children) {
    ...
    return [childArray as (T extends Iterable<React.ReactNode> ? Array<React.ReactNode> : T), 
      (childArray.some(x=>!!x) ? childArray.some(isCodeElem) : undefined)] // Parent node contains code elem if one of these (direct) children is a code element.
  }

  // Return TF = undefined if children is null/undefined, and otherwise TF = whether or not children is a code element
...
//   console.log('lastReturn.current:', lastReturn.current, lastChildren.current === grandchildren);
  ...
  const ret = lastReturn.current = (()=>{
    if(!grandchildren) return undefined;
    if(typeof grandchildren !== 'object' || !(Symbol.iterator in grandchildren)) {
      return [grandchildren, undefined] as unknown as SplitLines;
    }
    // console.log('Splitting child lines:', grandchildren);
    const [above, below] = splitChildLines(grandchildren);
    // if(!below) return undefined;
    // // if(!above) return true;
    console.log('Returning split lines:', [above, below])
...
const TransformedCodeElement = ({contentRef, above, below, children, ...props}: {contentRef: React.RefObject<HTMLDivElement | null> | ((el: HTMLDivElement | HTMLElement | null)=>void), above: SpanElems | undefined, below: SpanElems | undefined, } & React.ComponentProps<'code'>) => {
  if(!above || !below) {
    console.log('TCE:', above, below);
  }
  if(!below) return <code {...props}>{above}</code>;
  if(!above) return <CollapsibleContent className="CollapsibleContent" asChild>
    <code {...props}>{below}</code>
  </CollapsibleContent>;

  return <code {...props}>
    {above}
    <CollapsibleContent asChild className="CollapsibleContent pb-2em overflow-y-hidden overflow-x-visible m-0 p-0 inset-0 
                                        border-none bg-none outline-none shadow-none drop-shadow-none w-fit [overflow-inline:visible] [overflow-block:hidden]">
      <div ref={contentRef}>{below}</div>
    </CollapsibleContent>    
    {/* bg-blue-900/50 */}
...
const CollapsibleCodeInner = React.memo(({children: propsChildren, className, ...props}: React.HTMLAttributes<HTMLPreElement> & {'data-language'?: string}) => {
  ...
  const [above, below] = React.useMemo(()=>[
    codeTransformSpec?.[0], codeTransformSpec?.[1]], [codeTransformSpec]);
//   console.log('CCI', tf, children, codeProps, codeTransformSpec, above, below);
  ...
  if (!tf) {
    return <pre {...props} children={propsChildren} className={className} />;
  }

  const language = props['data-language']?.toUpperCase();
    
  return (
    // <div className="collapsible-code-wrapper w-full relative h-auto inset-0 p-0 m-0 border-none outline-none shadow-none bg-none">
      <Collapsible open={open} onOpenChange={setOpen} data-collapsible-open={open} disabled={!codeTransformSpec} className="collapsible-code" asChild>
        <div className="flex flex-col flex-nowrap mx-4 my-3 relative w-[calc(100%-8*var(--spacing))]
        ">
          ...
          <div className="bg-emerald-300 dark:bg-green-900 p-2 rounded-md">
            <pre {...props} className={cn(
              ...
              "text-nowrap",
              className)}>
                {/* Note that `codeProps` includes new children */}
...
export default CollapsibleCodeInner;